<template>
    <div class="trade-container">
        ...
        <div class="content">
            ...
            <div class="address clearFix" v-for="item of address" :key="item.id">
                <span class="username" :class="{ selected: item.isDefault === '1' }">{{ item.consignee }}</span>
                <p @click=" changDefaultAddress(item)">
            ...
            <div class="detail">
                <h5>商品清单</h5>
                <ul class="list clearFix" v-for="shop of shopInfo.detailArrayList" :key="shop.skuId">
                    <li>
        ...
        <div class="trade">
            ...
            <div class="receiveInfo">
                ...
                联系方式：<span>{{ defaultUser.phoneNum }}</span>
            </div>
        </div>
        <div class="sub clearFix">
            <a class="subBtn" href="javascript:void(0)" @click="submit">提交订单</a>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        ...mapState('shop', ['shopInfo', 'payId']),
        defaultUser() {
            return this.address.find((item) => item.isDefault === "1") || {}
        }
    },
    methods: {
        changDefaultAddress(user) {
        ...
        async submit() {
            const tradeNo = this.shopInfo.tradeNo
            const data = {
            ...
            try {
                await this.$store.dispatch("shop/submitInfo", {tradeNo, data})
                await this.$router.push({path: '/pay', query: {orderId: this.payId}})
            } catch (error) {
                alert(error.message);
            }
        }
